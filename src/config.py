...
# ── Porte qualité (S11 — evaluate.py) ────────────────────────────────────────
# Seuils minimaux pour valider un modèle avant promotion
# Surchargeables via variables d'environnement (ex. CI ou test de régression)
EVAL_ROC_AUC_MIN = float(os.environ.get("EVAL_ROC_AUC_MIN", "0.95"))
EVAL_F1_MIN      = float(os.environ.get("EVAL_F1_MIN", "0.40"))

# ── API (S15 — predict_client.py / predictions_dag.py) ───────────────────────
